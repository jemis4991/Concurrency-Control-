...
# it checks for conflicting locks and calls the wait-die routine to prevent deadlock situations.
    ...
    if length != 0:
        for i in range(length):
            if lockTableList[i].lockedDataItem == resourceName:
                ...
                if lockTableList[
                    i].lockMode == WL:
                    # This checks if the resource requested is under a writeLock by another transaction and if thats the case, wait die is called
                    print(
                        "Conflicting Write lock: data item " + resourceName + " is under ReadLock by Transaction %d" % (
                            lockTableList[i].lockHeldByTransaction[0]))
                    outputfile.write("Conflicting Write lock: data item " + str(
                        resourceName) + " is under ReadLock by Transaction " + str(
                        lockTableList[i].lockHeldByTransaction[0]))
                    print("calling wait-die mechanism")
                    outputfile.write("\ncalling wait-die mechanism. ")
                    waitDie(searchTransaction(transactionID),
                    ...
                    outputfile.write(
                        "Putting the data item " + str(resourceName) + " under ReadLock by Transaction " + str(
                            transactionID))
        ...
        if flag == 0:
            ...
            print("Putting the data item " + resourceName + " under ReadLock by Transaction %d" % transactionID)
            outputfile.write(
                "Putting the data item " + str(resourceName) + " under ReadLock by Transaction " + str(transactionID))
                ...
                if lockTableList[i].lockMode == RL:
                    if len(lockTableList[i].lockHeldByTransaction) == 1:
                        if lockTableList[i].lockHeldByTransaction[0] == transactionID:
                            ...
                            print(
                                "Upgrading Readlock to WriteLock on data item " + resourceName + " for transaction %d" % transactionID)
                            outputfile.write(
                                "Upgrading Readlock to WriteLock on data item " + resourceName + " for transaction %d. " % transactionID)

                        else:
                            print("data item " + resourceName + "is under ReadLock by %d transaction." % (
                            lockTableList[i].lockHeldByTransaction[0]))
                            outputfile.write("data item " + resourceName + "is under ReadLock by %d transaction. " % (
                            lockTableList[i].lockHeldByTransaction[0]))
                            print("call wait die")
                            outputfile.write("\ncall wait die. ")
                            # checks if the resource is under read lock by another transaction then call wait die
                            waitDie(searchTransaction(transactionID),
                        ...
                        for lockHeld in lockTableList[i].lockHeldByTransaction:
                            if lockHeld != transactionID:
                                print(
                                    "data item " + resourceName + " is under ReadLock by %d transaction." % (lockHeld))
                                outputfile.write(
                                    "data item " + resourceName + " is under ReadLock by %d transaction. " % (lockHeld))
                                print("call wait die")
                                outputfile.write("\ncall wait die. ")
                                # checks if requested resource is under readLock by multiple transactions then call the wait die method iteratively for all that transaction.
                                waitDie(searchTransaction(transactionID),
                    ...
                    print(
                        "Conflicting WriteLock: data item " + resourceName + " is under WriteLock by Transaction %d" % (
                            lockTableList[i].lockHeldByTransaction[0]))
                    outputfile.write(
                        "Conflicting WriteLock: data item " + resourceName + " is under WriteLock by Transaction %d . " % (
                            lockTableList[i].lockHeldByTransaction[0]))
                    print("call wait die")
                    # checks if resource is under writelock by another transaction call wait-die
                    outputfile.write("\ncall wait die. ")
                    waitDie(searchTransaction(transactionID),
                              searchTransaction(lockTableList[i].lockHeldByTransaction[0]), lockTableList[i], operation)
        if flag == 0:
            lockTableList.append(
    ...
    for transaction in transactionTableList:
        if transaction.transactionID == transactionNumber and transaction.transactionState != A:
            ...
            print("Transaction " + str(
                transactionNumber) + " is already aborted or commited. So no changes in the tables.")
            outputfile.write(
                "\nTransaction " + str(
                    transactionNumber) + " is already aborted or commited. So no changes in the tables.\n")
        ...
        for blockedBy in transaction.blockedBy:
            if blockedBy == transactionID:
                print(
                    ...
                    str(blockedBy))
                outputfile.write("\ntransacation is matched " + str(
                    transaction.transactionID) + " which is blocked by transaction " +
                                 str(blockedBy))
...
# the wait die function deadlock prevention mechanism
def waitDie(requestingTransaction, holdingTransaction, lockedResource, operation):
    if requestingTransaction.timeStamp < holdingTransaction.timeStamp:
        ...
        requestingTransaction.changeTransactionState(A)
        print("time stamp of requestion-transaction" + str(requestingTransaction.transactionID) +
              "is greater than resource holding-transaction" + str(holdingTransaction.transactionID))
        outputfile.write("\ntime stamp of requestion-transaction " + str(requestingTransaction.transactionID)
                         + " is greater than resource holding-transaction " + str(holdingTransaction.transactionID))
        print("Aborting Transaction %d" % requestingTransaction.transactionID)
        outputfile.write("\nAborting Transaction %d. " % requestingTransaction.transactionID)
        unlock(requestingTransaction.transactionID)
        # if checkDuplicateOperation(operation, holdingTransaction):
    ...
    if length != 0:
        for i in range(0, length):
            if transactionTableList[i].transactionID == transactionID and transactionTableList[i].transactionState == W:
                outputfile.write(
                    "transaction state of tranaction " + str(transactionTableList[i].transactionID) + " waiting.\nso "
                ...
                operation = ""
                writeToFile()
            elif transactionTableList[i].transactionID == transactionID and (
                    transactionTableList[i].transactionState == A or transactionTableList[i].transactionState == C):
                operation = ""
                print("Operation Ignored. Transaction " + str(
                    transactionID) + " is already aborted or commited. So no changes in the tables. ")
                outputfile.write("\nTransaction " + str(
                    transactionID) + " is already aborted or commited. So no changes in the tables.\n")
